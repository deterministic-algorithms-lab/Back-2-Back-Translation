...
class xlmb2b(nn.Module, model_utils):
    ...
    def choose(self) :
        ...
        i = torch.tensor([i for i in range(y.shape[0])],device=self.device)
    ...
    def forward(self, dat, already_embed = False) :
        ...
        if self.pll_data :
            ...
            self.tgt_key_pad_mask = torch.zeros((self.bs, self.max_tr_seq_len),device=self.device)
            ...
            self.tr_embd = torch.zeros((self.bs, self.max_tr_seq_len, self.d_model),dtype=torch.float64, device=self.device)
            self.not_done_samples = torch.ones(self.bs, dtype=torch.bool, device=self.device)
            self.it_no = 0                                                           #if nth word of target sequence is being predicted,
            self.final_out = []                                                      #then iteration number(it_no) == n-1
            self.lengs = torch.zeros((self.bs),device=self.device)
            self.actual_bs = int(self.bs/self.beam_size)
            self.prev_probs = torch.zeros((self.actual_bs,self.max_tr_seq_len+1,self.beam_size), device=self.device, dtype=torch.float64)
            self.tgt_key_pad_mask[:,self.it_no] = torch.ones((self.bs), device=self.device)
            self.just_now_completed_samples_mask = torch.zeros((self.bs), dtype=torch.bool, device=self.device)
            ...
            while True :
                ...
                trfrmr_out = torch.zeros((self.bs,self.vocab_size), device=self.device, dtype=torch.float64)
                ...
                output_at_it_no = torch.zeros((self.bs,1), dtype=torch.long, device=self.device)
