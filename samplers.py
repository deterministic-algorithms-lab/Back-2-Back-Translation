...
class simpler_sampler() :
    ...
    @staticmethod
    def get_dtalodr_for_dic(dic, lang='en') :
        df = pd.DataFrame.from_dict(dic, orient='index')
        df.rename(columns = {'0':lang})
        ds = mono_datst(df, lang)
        dl = DataLoader(ds, batch_size = b_sz, coll_fn = partial(coll, pll_dat=False))
        return dl
...
class beam_search(simpler_sampler):
    ...
    def nuc_sample(self, vec, t, token_id = True, calc_embdngs=True) :
        ...
        else : xlm_embdngs, probs = get_probs(vec,t,!token_id,calc_embdngs)
        ...
        for i in range(len(att_mask.shape[0])) :
            ...
            for j in range(len(att_mask.shape[1])) :
                if att_mask[i][j] == True  and att_mask[i][j+1] != True :
                    for_1_sample.append(self.nuc_sample(model_out[i][j], t, token_id=!get_xlm_embdngs, calc_embdngs))
                else :
                    if get_xlm_embdngs :
